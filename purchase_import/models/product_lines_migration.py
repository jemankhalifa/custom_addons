...
class AccountMoveLine(models.Model):
    ...
    purchase_order_line_id = fields.Many2one('purchase.order.line.data', string='Purchase Order Line (Custom)')
...
class OrderLineData(models.Model):
    ...
    taxes_id = fields.Many2many('account.tax', string='Taxes', store=True,
    ...
    invoice_lines = fields.One2many('account.move.line', 'purchase_order_line_id', string="Bill Lines", readonly=True,
                                    copy=False)
    ...
    def _compute_amount(self):
        for line in self:
            ...
            line.update({
                ...
                'price_subtotal': taxes['total_excluded'] - all_tax - (
                        taxes['total_excluded'] * (self.discount / 100)), })
            ...
            for inv_line in line.invoice_lines:
                if inv_line.move_id.state not in ['cancel']:
                    if inv_line.move_id.move_type == 'in_invoice':
                        qty += inv_line.uom_id._compute_quantity(inv_line.quantity, line.product_uom_id)
